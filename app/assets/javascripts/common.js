...
function doAction(action){
	...
	var loc = "actions.php?" + paramsArr.join("&");
...
function setUpPadSwitches(){
	$('[padSwitch]').click(function(e){
		var target = $(this).attr("padTarget");
		target = target ? $('#' + target).get(0) : $(this).get(0);
		showPad($(this).attr('padSwitch'), target , eval($(this).attr('padCallback')), e);
	})
...
function padItemSelected(targetElem, padItemValue, padItemText){
	var container = null;
	var name = "";
	targetElem.innerHTML = padItemText;
	targetElem.value = padItemValue; // id of whatever was selected (key, type, group etc)
	return true;
}
